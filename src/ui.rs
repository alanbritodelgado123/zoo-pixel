...
impl UiRenderer {
                          ...
                          top: f32, bottom: f32, hint_z: &str)
    {
        ...
        draw_text_ex(&hint, (sw - htw) / 2.0, bottom - 8.0,
            TextParams { font: Some(&self.font), font_size: fs_h,
                          color: COLOR_DIM, ..Default::default() });
    }
    ...
    fn render_inicio(&self, estado: &Estado) {
        ...
        draw_text_ex(sub, (sw - sw2) / 2.0, sh * 0.22 + th_t * 0.8,
            TextParams { font: Some(&self.font), font_size: fs_sub,
                          color: COLOR_TEXT_DIM, ..Default::default() });

        // Opciones — bloque alineado a la izquierda, centrado como grupo
        ...
        // Ancho máximo de las opciones (sin cursor)
        let max_w = opciones.iter()
            .map(|opt| measure_text(opt, Some(&self.font), fs_m, 1.0).width)
            .fold(0.0_f32, f32::max);
        let cursor_w = measure_text("> ", Some(&self.font), fs_m, 1.0).width;

        // Centrar el bloque (texto sin cursor)
        let block_x = (sw - max_w) / 2.0;
        ...
        for (i, opt) in opciones.iter().enumerate() {
            ...
            if selected {
                draw_text_ex(">", block_x - cursor_w, y,
                    TextParams { font: Some(&self.font), font_size: fs_m,
                                  color: COLOR_ACCENT, ..Default::default() });
            }
            draw_text_ex(opt, block_x, y,
                TextParams { font: Some(&self.font), font_size: fs_m,
    ...
    fn render_config(&self, estado: &Estado) {
        ...
        // Opciones
        let fs_c = fs_adaptativo("Volumen Efectos", &self.font, fs_pct(0.032), sw * 0.7);
        ...
        // Centrar bloque de opciones
        let max_w = MenuConfig::OPCIONES.iter()
            .map(|opt| measure_text(opt, Some(&self.font), fs_c, 1.0).width)
            .fold(0.0_f32, f32::max);
        ...
        for (i, opt) in MenuConfig::OPCIONES.iter().enumerate() {
            ...
            if selected {
                draw_text_ex(">", block_x - cursor_w, y + th_c,
                    TextParams { font: Some(&self.font), font_size: fs_c,
                                  color: COLOR_ACCENT, ..Default::default() });
            }
            draw_text_ex(opt, block_x, y + th_c,
                TextParams { font: Some(&self.font), font_size: fs_c,
                              color, ..Default::default() });

            match i {
                0 => self.render_barra_vol(sw, y + th_c + 8.0, bar_h_px,
                         mc.volumen_musica, selected, fs_v),
                1 => self.render_barra_vol(sw, y + th_c + 8.0, bar_h_px,
        ...
        let htw = measure_text(hint, Some(&self.font), fs_h, 1.0).width;
        draw_text_ex(hint, (sw - htw) / 2.0, sh * 0.90,
            TextParams { font: Some(&self.font), font_size: fs_h,
                          color: COLOR_DIM, ..Default::default() });
    }
    ...
    fn render_mapa_completo(&self, estado: &Estado) {
        ...
        // Nombre de la zona bajo el cursor
        ...
        let fs_h = fs_adaptativo(hint, &self.font, fs_pct(0.022), sw * 0.95);
        let htw = measure_text(hint, Some(&self.font), fs_h, 1.0).width;
        draw_text_ex(hint, (sw - htw) / 2.0, sh - 8.0,
            TextParams { font: Some(&self.font), font_size: fs_h,
                          color: COLOR_DIM, ..Default::default() });
    }
    ...
    fn render_libreta(&self, estado: &Estado) {
        ...
        // Info de animal seleccionado
        if let Some(ref info) = estado.libreta_info {
            self.render_info_animal(
                &info.nombre_comun, &info.nombre_cientifico, &info.descripcion,
                info.texto_pos, info.terminado, st, sh, "Z: Cerrar",
            );
            return;
        }
        ...
        let fs_h = fs_adaptativo(hint, &self.font, fs_pct(0.022), sw * 0.95);
        let htw = measure_text(hint, Some(&self.font), fs_h, 1.0).width;
        draw_text_ex(hint, (sw - htw) / 2.0, sh - 10.0,
            TextParams { font: Some(&self.font), font_size: fs_h,
                          color: COLOR_DIM, ..Default::default() });
    }
    ...
    fn render_juego(&self, estado: &Estado, fondos: &Fondos) {
        ...
        if let Some(ref info) = estado.info_overlay {
            ...
            self.render_info_animal(
                &info.nombre_comun, &info.nombre_cientifico, &info.descripcion,
                info.texto_pos, info.terminado, content_top, sh, "Z: Continuar",
            );
            return;
        }
        ...
        // Minimapa solo en modo normal sin info ni eventos
    ...
    fn render_normal(&self, estado: &Estado, content_top: f32, content_bottom: f32) {
        ...
        if !estado.escena.es_entrada() {
            ...
            let htw = measure_text(hint, Some(&self.font), fs_h, 1.0).width;
            draw_text_ex(hint, (sw - htw) / 2.0, content_bottom - 5.0,
                TextParams { font: Some(&self.font), font_size: fs_h,
                              color: COLOR_DIM, ..Default::default() });
        }
    }
                        ...
                        content_top: f32, content_bottom: f32)
    {
        ...
        let htw = measure_text(hint, Some(&self.font), fs_h, 1.0).width;
        draw_text_ex(hint, (sw - htw) / 2.0, content_bottom - 8.0,
            TextParams { font: Some(&self.font), font_size: fs_h,
                          color: COLOR_DIM, ..Default::default() });
    }

    fn render_foto_grid(&self, animales: &[crate::db::Animal], _indice: usize,
                        celda: usize, ya_vistos: &std::collections::HashSet<usize>,
                        content_top: f32, content_bottom: f32)
    {
        let sw = screen_width();
        draw_rectangle(0.0, content_top, sw, content_bottom - content_top, COLOR_BG_DARK);
        ...
        for c in 0..4_usize {
            ...
            if c == celda {
                ...
                draw_text_ex("?",
                    cx + (grid_size - 4.0 - btw) / 2.0, cy + grid_size * 0.6,
    ...
    fn render_pesca(&self, estado: &Estado, content_top: f32, content_bottom: f32) {
        ...
        match pesca.fase {
            ...
            FasePesca::InfoPez => {
                // Se muestra via info_overlay, no debería llegar aquí
            }
        }
    }
    ...
    fn render_museo(&self, estado: &Estado, content_top: f32, content_bottom: f32) {
        ...
        match museo.fase {
            ...
            FaseMuseo::Explorando => {
                ...
                let color_exc = if sel_e { COLOR_WARM } else { COLOR_TEXT_DIM };
                if sel_e {
                ...
                let color_q = if sel_q { COLOR_SPECIAL } else { COLOR_TEXT_DIM };
                if sel_q {
                         ...
                         content_top: f32, content_bottom: f32)
    {
        ...
        for row in 0..museo.grilla_rows {
            for col in 0..museo.grilla_cols {
                ...
                draw_rectangle(x + 1.0, y + 1.0, cell_size - 2.0,
                    cell_size - 2.0, bg);
                ...
                if is_cursor {
                    ...
                    draw_rectangle_lines(x + 1.0, y + 1.0, cell_size - 2.0,
                        cell_size - 2.0, 1.0, bc);
                }
                ...
                if museo.grilla[row][col] == CeldaExcavacion::Fosil {
                    ...
                    draw_text_ex("*", x + (cell_size - itw) / 2.0,
                        y + cell_size * 0.65,
                ...
                if let CeldaExcavacion::Roca(c) = museo.grilla[row][col] {
                    ...
                    draw_text_ex(&num, x + (cell_size - ntw) / 2.0,
                        y + cell_size * 0.6,
                        TextParams { font: Some(&self.font), font_size: fs_n,
        ...
        draw_text_ex(hint, (sw - htw) / 2.0, content_bottom - 8.0,
            TextParams { font: Some(&self.font), font_size: fs_hi,
                          color: COLOR_DIM, ..Default::default() });
    }
                   ...
                   content_top: f32, content_bottom: f32)
    {
        ...
        self.render_texto_wrapped(&museo.quiz_pregunta, sw * 0.05,
            content_top + 20.0, sw * 0.9, content_top + 100.0, fs_q, COLOR_TEXT);
                     ...
                     mostrar_info: bool, content_bottom: f32)
    {
        ...
        if mostrar_info {
            ...
            draw_text_ex(hint, sw - htw - 15.0, banner_y + banner_h * 0.7,
    ...
    // Dibujar solo las celdas que existen (sin fondo rectangular grande)
    for escena in Escena::TODAS {
        let (c, r) = escena.pos_mapa();
        ...
        // Fondo semitransparente individual por celda (sombra sutil)
        draw_rectangle(x - 1.0, y - 1.0, cell + 2.0, cell + 2.0,
            Color::new(0.0, 0.0, 0.0, 0.4));

        // Celda con color sólido
    ...
    // Conexiones visibles (líneas sutiles entre celdas visibles)
    for escena in Escena::TODAS {
        let (c, r) = escena.pos_mapa();
        let dc = c as i32 - centro_c;
        let dr = r as i32 - centro_r;
        ...
        for conexion in escena.conexiones().iter().flatten() {
            ...
            let x2 = map_x + (dc2 + radio) as f32 * (cell + gap) + cell / 2.0;
            let y2 = map_y + (dr2 + radio) as f32 * (cell + gap) + cell / 2.0;
