...
impl UiRenderer {
    ...
    fn render_info_animal(&self, nombre: &str, cientifico: &str, desc: &str, texto_pos: usize, terminado: bool, top: f32, bottom: f32) {
        let sw = screen_width();
        ...
        draw_text_ex(nombre, (sw - ntw) / 2.0, name_y + th_name, TextParams {
        ...
        draw_text_ex(cientifico, (sw - stw) / 2.0, sci_y + th_sci, TextParams {
        ...
        // Descripcion en verde
        let fs_desc = fs_pct(0.026);
        let desc_text: String = desc.chars().take(texto_pos).collect();
        self.render_texto_wrapped(&desc_text, sw * 0.08, sep_y + 8.0, sw * 0.84, bottom - 30.0, fs_desc, COLOR_GREEN);
        ...
        draw_text_ex(hint, (sw - htw) / 2.0, bottom - 8.0, TextParams {
    ...
    fn render_inicio(&self, estado: &Estado) {
        ...
        draw_text_ex(titulo, (sw - tw) / 2.0, sh * 0.22, TextParams { font: Some(&self.font), font_size: fs_t, color: COLOR_ACCENT, ..Default::default() });
        ...
        draw_text_ex(sub, (sw - sw2) / 2.0, sh * 0.22 + th * 0.8, TextParams { font: Some(&self.font), font_size: fs_sub, color: COLOR_TEXT_DIM, ..Default::default() });
        ...
        for (i, opt) in opciones.iter().enumerate() {
            ...
            draw_text_ex(&texto, (sw - tw) / 2.0, y_base + i as f32 * item_h, TextParams { font: Some(&self.font), font_size: fs_m, color, ..Default::default() });
        }
    }
    ...
    fn render_config(&self, estado: &Estado) {
        ...
        draw_text_ex(titulo, (sw - tw) / 2.0, sh * 0.12 + th_t, TextParams { font: Some(&self.font), font_size: fs_t, color: COLOR_ACCENT, ..Default::default() });
        ...
        for (i, opt) in MenuConfig::OPCIONES.iter().enumerate() {
            ...
            draw_text_ex(&texto, (sw - ttw) / 2.0, y + th_c, TextParams { font: Some(&self.font), font_size: fs_c, color, ..Default::default() });
            match i {
                0 => self.render_barra_vol(sw, y + th_c + 8.0, bar_h_px, mc.volumen_musica, selected, fs_v),
                1 => self.render_barra_vol(sw, y + th_c + 8.0, bar_h_px, mc.volumen_efectos, selected, fs_v),
                _ => {}
            }
        }
        ...
        let htw = measure_text(hint, Some(&self.font), fs_h, 1.0).width;
        draw_text_ex(hint, (sw - htw) / 2.0, sh * 0.90, TextParams { font: Some(&self.font), font_size: fs_h, color: COLOR_DIM, ..Default::default() });
    }

fn render_barra_vol(&self, sw: f32, y: f32, bar_h: f32, valor: f32, selected: bool, fs_v: u16) {
    ...
    fn render_mapa_completo(&self, estado: &Estado) {
        clear_background(COLOR_BG_DARK);
        let sw = screen_width(); let sh = screen_height(); let st = safe_top();
        ...
        let tw = measure_text(titulo, Some(&self.font), fs_t, 1.0).width;
        draw_text_ex(titulo, (sw - tw) / 2.0, title_y, TextParams { font: Some(&self.font), font_size: fs_t, color: COLOR_ACCENT, ..Default::default() });
        ...
        for escena in Escena::TODAS {
            ...
            draw_text_ex(letra, nx + (size - ltw) / 2.0, ny + size * 0.62, TextParams { font: Some(&self.font), font_size: fs_n, color: COLOR_TEXT, ..Default::default() });
        }
        ...
        draw_text_ex(cursor_name, (sw - inf_tw) / 2.0, sh - 30.0, TextParams { font: Some(&self.font), font_size: fs_info, color: COLOR_TEXT, ..Default::default() });

        let puede_ir = estado.mapa_cursor != estado.escena && estado.visitadas.contains(&estado.mapa_cursor);
        let hint = if puede_ir { "Flechas: mover  Z: ir  X: cerrar" } else { "Flechas: mover  X: cerrar" };
        let fs_h = fs_adaptativo(hint, &self.font, fs_pct(0.022), sw * 0.95);
        let htw = measure_text(hint, Some(&self.font), fs_h, 1.0).width;
        draw_text_ex(hint, (sw - htw) / 2.0, sh - 8.0, TextParams { font: Some(&self.font), font_size: fs_h, color: COLOR_DIM, ..Default::default() });
    }
    ...
    fn render_libreta(&self, estado: &Estado) {
        ...
        if let Some(ref info) = estado.libreta_info {
            self.render_info_animal(&info.nombre_comun, &info.nombre_cientifico, &info.descripcion, info.texto_pos, info.terminado, st, sh);
            return;
        }
        ...
        if total == 0 {
            ...
            draw_text_ex(msg, (sw - mtw) / 2.0, sh * 0.5, TextParams { font: Some(&self.font), font_size: fs_m, color: COLOR_TEXT_DIM, ..Default::default() });
        } else {
            let por_pagina = 5;
            ...
            for (i, idx) in (inicio..fin).enumerate() {
                ...
                let color = if selected { COLOR_ACCENT } else { COLOR_TEXT };
                let prefix = if selected { "> " } else { "  " };
                let texto = format!("{}{}", prefix, entry.nombre);
                let name_fs = fs_adaptativo(&texto, &self.font, fs_name, sw - 30.0);
                draw_text_ex(&texto, 15.0, y + th_name, TextParams { font: Some(&self.font), font_size: name_fs, color, ..Default::default() });
                draw_line(15.0, y + item_h - 4.0, sw - 15.0, y + item_h - 4.0, 1.0, Color::new(0.3, 0.3, 0.3, 0.4));
            }
            ...
            draw_text_ex(&pag, (sw - ptw) / 2.0, sh - 35.0, TextParams { font: Some(&self.font), font_size: fs_p, color: COLOR_TEXT_DIM, ..Default::default() });
        }

        let hint = "Z: Ver  Flechas: Navegar  X: Cerrar";
        let fs_h = fs_adaptativo(hint, &self.font, fs_pct(0.022), sw * 0.95);
        let htw = measure_text(hint, Some(&self.font), fs_h, 1.0).width;
        draw_text_ex(hint, (sw - htw) / 2.0, sh - 10.0, TextParams { font: Some(&self.font), font_size: fs_h, color: COLOR_DIM, ..Default::default() });
    }

    // ─── Juego ───
    fn render_juego(&self, estado: &Estado, fondos: &Fondos) {
        let sw = screen_width(); let sh = screen_height();
        let bar_h = bar_height(); let st = safe_top(); let sb = safe_bottom();
        let content_top = st + bar_h;

        // Info overlay (pesca/museo resultado) - pantalla completa
        if let Some(ref info) = estado.info_overlay {
            clear_background(COLOR_BG_DARK);
            self.render_barra_superior(estado, sw, bar_h, st);
            self.render_info_animal(&info.nombre_comun, &info.nombre_cientifico, &info.descripcion, info.texto_pos, info.terminado, content_top, sh);
            return;
        }
        ...
        match &estado.modo {
            ...
            ModoVista::Seleccion { animales, indice } => self.render_seleccion(animales, *indice, content_top, sh),
            ModoVista::ViendoAnimal { animal, texto_pos, terminado, .. } => {
                self.render_info_animal(&animal.nombre_comun, &animal.nombre_cientifico, &animal.descripcion, *texto_pos, *terminado, content_top, sh);
            }
            ModoVista::Foto { animales, indice_actual, celda, foto_tomada, texto_pos, terminado, ya_vistos, .. } => {
                if *foto_tomada {
                    let a = &animales[*indice_actual];
                    self.render_info_animal(&a.nombre_comun, &a.nombre_cientifico, &a.descripcion, *texto_pos, *terminado, content_top, sh);
                } else {
        ...
        if let Some(ref evento) = estado.eventos.evento_actual {
            if !en_info { self.render_evento(evento, estado.eventos.mostrar_info, sb); }
        }
        ...
        // Minimapa solo en normal sin info
    ...
    fn render_barra_superior(&self, estado: &Estado, sw: f32, bar_h: f32, st: f32) {
        ...
        draw_text_ex(estado.escena.nombre(), 8.0, st + bar_h * 0.72, TextParams { font: Some(&self.font), font_size: fs, color: COLOR_TEXT, ..Default::default() });
        let fase = estado.ciclo.nombre_fase();
        let fs_f = fs_pct(0.02);
        let ftw = measure_text(fase, Some(&self.font), fs_f, 1.0).width;
        draw_text_ex(fase, sw - ftw - 8.0, st + bar_h * 0.72, TextParams { font: Some(&self.font), font_size: fs_f, color: COLOR_DIM, ..Default::default() });
    }
    ...
    fn render_normal(&self, estado: &Estado, content_top: f32, content_bottom: f32) {
        ...
        if conns[0].is_some() { let a = "^"; let aw = measure_text(a, Some(&self.font), fs_arrow, 1.0).width; draw_text_ex(a, (sw - aw) / 2.0, content_top + 50.0, TextParams { font: Some(&self.font), font_size: fs_arrow, color: COLOR_HIGHLIGHT, ..Default::default() }); }
        if conns[1].is_some() { let a = "v"; let aw = measure_text(a, Some(&self.font), fs_arrow, 1.0).width; draw_text_ex(a, (sw - aw) / 2.0, content_bottom - 20.0, TextParams { font: Some(&self.font), font_size: fs_arrow, color: COLOR_HIGHLIGHT, ..Default::default() }); }
        if conns[2].is_some() { draw_text_ex("<", 10.0, mid_y, TextParams { font: Some(&self.font), font_size: fs_arrow, color: COLOR_HIGHLIGHT, ..Default::default() }); }
        if conns[3].is_some() { let a = ">"; let aw = measure_text(a, Some(&self.font), fs_arrow, 1.0).width; draw_text_ex(a, sw - aw - 10.0, mid_y, TextParams { font: Some(&self.font), font_size: fs_arrow, color: COLOR_HIGHLIGHT, ..Default::default() }); }
    ...
    fn render_seleccion(&self, animales: &[crate::db::Animal], indice: usize, content_top: f32, content_bottom: f32) {
        ...
        for (i, animal) in animales.iter().enumerate() {
            ...
            let txt_fs = fs_adaptativo(&texto, &self.font, fs_name, sw - 30.0);
            draw_text_ex(&texto, 15.0, y_start + i as f32 * item_h + th, TextParams { font: Some(&self.font), font_size: txt_fs, color, ..Default::default() });
        }
    ...
    fn render_foto_grid(&self, animales: &[crate::db::Animal], _indice: usize, celda: usize, ya_vistos: &std::collections::HashSet<usize>, content_top: f32, content_bottom: f32) {
        let sw = screen_width();
        draw_rectangle(0.0, content_top, sw, content_bottom - content_top, COLOR_BG_DARK);
        let mid_y = (content_top + content_bottom) / 2.0;
    ...
    fn render_pesca(&self, estado: &Estado, content_top: f32, content_bottom: f32) {
        ...
        draw_rectangle(0.0, content_top, sw, content_bottom - content_top, COLOR_BG_DARK);
        let fs_p = fs_pct(0.035); let fs_h = fs_pct(0.022);
        let mid_y = (content_top + content_bottom) / 2.0;
        ...
        match pesca.fase {
            ...
            FasePesca::Resultado => {
                ...
                let hint = "Z: Siguiente  X: Salir";
                let htw = measure_text(hint, Some(&self.font), fs_h, 1.0).width;
                draw_text_ex(hint, (sw - htw) / 2.0, content_bottom - 8.0, TextParams { font: Some(&self.font), font_size: fs_h, color: COLOR_DIM, ..Default::default() });
            }
            FasePesca::InfoPez => {
                // Info se muestra via info_overlay, no debería llegar aquí
            }
        }
    }
        ...
        match museo.fase {
            ...
            FaseMuseo::Explorando => {
                ...
                for (i, dino) in museo.exhibiciones.iter().enumerate() {
                    ...
                    draw_text_ex(&format!("{}{}", prefix, dino.nombre), 20.0, y_start + i as f32 * item_h + th, TextParams { font: Some(&self.font), font_size: fs_item, color, ..Default::default() });
                }
                let exc_idx = museo.exhibiciones.len();
                let sel_e = museo.indice == exc_idx;
                let prefix = if sel_e { "> " } else { "  " };
                draw_text_ex(&format!("{}Excavar Fosil", prefix), 20.0, y_start + exc_idx as f32 * item_h + th, TextParams { font: Some(&self.font), font_size: fs_item, color: if sel_e { COLOR_WARM } else { COLOR_TEXT_DIM }, ..Default::default() });
                let quiz_idx = exc_idx + 1;
                let sel_q = museo.indice == quiz_idx;
                let prefix = if sel_q { "> " } else { "  " };
                draw_text_ex(&format!("{}Quiz", prefix), 20.0, y_start + quiz_idx as f32 * item_h + th, TextParams { font: Some(&self.font), font_size: fs_item, color: if sel_q { COLOR_SPECIAL } else { COLOR_TEXT_DIM }, ..Default::default() });
            ...
            _ => {} // FosilRevelado y ViendoExhibicion handled via info_overlay
        }
    }

    fn render_excavacion(&self, museo: &MinijuegoMuseo, content_top: f32, content_bottom: f32) {
        let sw = screen_width(); let fs_h = fs_pct(0.022);
    ...
    fn render_quiz(&self, museo: &MinijuegoMuseo, content_top: f32, content_bottom: f32) {
        let sw = screen_width();
        let fs_q = fs_pct(0.028); let fs_h = fs_pct(0.022);
        ...
        for (i, opt) in museo.quiz_opciones.iter().enumerate() {
            ...
            let color = if museo.quiz_respondida {
                if i == museo.quiz_correcta { COLOR_GREEN } else if sel && !museo.quiz_correcta_resp { COLOR_DANGER } else { COLOR_TEXT_DIM }
    ...
    fn render_dialogo(&self, estado: &Estado) {
        ...
        draw_text_ex(personaje, box_x + padding, box_y + padding + th_p, TextParams { font: Some(&self.font), font_size: fs_p, color: COLOR_ACCENT, ..Default::default() });
        ...
        for line in lines.iter().skip(start) {
            ...
            cy += line_h;
            draw_text_ex(line, box_x + padding, cy, TextParams { font: Some(&self.font), font_size: fs_d, color: COLOR_TEXT, ..Default::default() });
        }
        if estado.dialogo.terminado_linea {
            draw_text_ex("v", box_x + box_w - 20.0, box_y + box_h - 8.0, TextParams { font: Some(&self.font), font_size: fs_pct(0.02), color: COLOR_ACCENT, ..Default::default() });
        }
    }
    ...
    fn render_evento(&self, evento: &crate::eventos::EventoAleatorio, mostrar_info: bool, content_bottom: f32) {
        ...
        if mostrar_info {
            ...
            self.render_texto_wrapped(&evento.detalle, 18.0, box_y + 8.0 + th + 8.0, sw - 36.0, box_y + box_h - 8.0, fs_pct(0.022), COLOR_TEXT);
        } else {
            let banner_h = 28.0;
    ...
    fn render_minimapa(&self, estado: &Estado, content_bottom: f32) {
        let cell = mini_size();
        let gap = mini_gap();
        let radio = 2_i32; // Celdas visibles alrededor de la actual
        ...
        draw_rectangle(map_x - 2.0, map_y - 2.0, total_w + 4.0, total_h + 4.0, Color::new(0.0, 0.0, 0.0, 0.5));
    ...
    fn render_texto_wrapped(&self, texto: &str, x: f32, y: f32, max_w: f32, max_y: f32, fs: u16, color: Color) {
        ...
        for line in self.word_wrap(texto, fs, max_w) {
            cy += line_h;
            if cy > max_y { break; }
            draw_text_ex(&line, x, cy, TextParams { font: Some(&self.font), font_size: fs, color, ..Default::default() });
        }
    }
    ...
    fn word_wrap(&self, texto: &str, fs: u16, max_w: f32) -> Vec<String> {
        ...
        for word in texto.split_whitespace() {
            ...
            if w > max_w && !current_line.is_empty() { lines.push(current_line); current_line = word.to_string(); }
            else { current_line = test; }
        }
